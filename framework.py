	...
	def Step(self, settings):
		...
		if self.renderer:
			...
			if self.bombSpawning:
				self.renderer.DrawPoint(self.renderer.to_screen(self.bombSpawnPoint), settings.pointSize,
										self.colors['bomb_center'])
				self.renderer.DrawSegment(self.renderer.to_screen(self.bombSpawnPoint),
										  self.renderer.to_screen(self.mouseWorld), self.colors['bomb_line'])
			...
			if settings.drawStats:
				...
				if self.t_draws and self.t_steps:
					...
					sum(self.t_draws) / len(self.t_draws), sum(self.t_steps) / len(self.t_steps)))
...
try:
	framework_module = __import__('%s_framework' % (fwSettings.backend.lower()),
								  fromlist=['%sFramework' % fwSettings.backend.capitalize()])
